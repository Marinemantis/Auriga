...
function Nav({ heroVisible }: { heroVisible: boolean }) {
  ...
  const scrolled = !heroVisible; // solid dark bg after hero scrolls away

  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-[#080808]/90 backdrop-blur-md border-b border-[#1A1A1A]" : "bg-transparent"}`}>
      <div className="max-w-[1320px] mx-auto px-6 lg:px-10 h-[72px] flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-7">
          ...
          <div className="relative group">
            <span
              className="flex items-center gap-1 cursor-default text-[11px] tracking-[0.12em] uppercase transition-colors duration-300 select-none text-white/70 group-hover:text-white"
            ...
            <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
              <div className="w-52 border shadow-xl py-1.5 bg-[#0d0d0d] border-[#222]">
                {[
                  ...
                  <Link key={href} href={href}
                    className="block px-5 py-2.5 text-[10px] tracking-[0.15em] uppercase transition-colors duration-200 text-[#F5F0E8]/60 hover:text-[#C8903A] hover:bg-[#161616]"
                    style={{ fontFamily: "var(--font-inter), sans-serif" }}
                  >
                    {label}
                  </Link>
          ...
          {["Destinations", "Experiences", "About", "Contact"].map(l => (
            <Link key={l} href={l === "Destinations" ? "/tours" : l === "Experiences" ? "/experiences" : l === "About" ? "/about" : "/contact"}
              className="text-[11px] tracking-[0.12em] uppercase transition-colors duration-300 text-white/70 hover:text-white"
              style={{ fontFamily: "var(--font-inter), sans-serif" }}
            >{l}</Link>
          ))}
        </nav>
        ...
        <div className="hidden md:flex items-center gap-5">
          <a href="https://wa.me/923339555682" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-[11px] transition-colors duration-300 text-white/60 hover:text-[#25D366]" style={{ fontFamily: "var(--font-inter), sans-serif" }}>
        ...
        <button className="md:hidden p-2 flex flex-col gap-1.5" onClick={() => setMenuOpen(!menuOpen)}>
          {[0,1,2].map(i => (
            <span key={i} className={`block h-px w-5 bg-white transition-all duration-300 ${i===0&&menuOpen?"rotate-45 translate-y-[7px]":""} ${i===1&&menuOpen?"opacity-0":""} ${i===2&&menuOpen?"-rotate-45 -translate-y-[7px]":""}`}/>
          ))}
        </button>
      </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div initial={{ opacity:0,height:0 }} animate={{ opacity:1,height:"auto" }} exit={{ opacity:0,height:0 }}
            className="md:hidden bg-[#080808] border-t border-[#1A1A1A] px-6 pb-8 pt-6 flex flex-col gap-5"
          >
            {["Destinations","Experiences","About","Contact"].map(l=>(
              <Link key={l} href={l === "Destinations" ? "/tours" : l === "Experiences" ? "/experiences" : l === "About" ? "/about" : "/contact"} onClick={()=>setMenuOpen(false)} className="text-sm tracking-[0.12em] uppercase text-[#F5F0E8]/70 hover:text-[#C8903A]" style={{ fontFamily:"var(--font-inter),sans-serif" }}>{l}</Link>
            ))}
            <Link href="/enquire" target="_blank" rel="noopener noreferrer" onClick={()=>setMenuOpen(false)} className="mt-2 text-center px-5 py-3 border border-[#C8903A] text-[#C8903A] text-xs tracking-[0.12em] uppercase hover:bg-[#C8903A] hover:text-[#080808] transition-all duration-300" style={{ fontFamily:"var(--font-inter),sans-serif" }}>Plan Your Journey</Link>
          </motion.div>
        )}
      </AnimatePresence>
    </header>
    ...
    <section className="bg-[#080808] overflow-hidden">
      <div className="grid lg:grid-cols-2 min-h-[560px]">
        ...
        >
          ...
          <a href="#contact"
            className="inline-flex items-center justify-center w-fit px-7 py-3.5 border border-[#C8903A] text-[#C8903A] text-[11px] tracking-[0.18em] uppercase font-semibold hover:bg-[#C8903A] hover:text-[#080808] transition-all duration-300"
            style={{ fontFamily:"var(--font-inter),sans-serif" }}
          >Watch the Film</a>
        </motion.div>
        ...
        >
          <Image
            ...
            alt="Auriga founders" fill className="object-cover" sizes="(max-width: 1024px) 100vw, 50vw"
          />
          <div className="absolute inset-0 bg-black/10" />
          {/* Play button overlay */}
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="rounded-full bg-white/20 backdrop-blur-sm border border-white/40 flex items-center justify-center hover:bg-white/30 cursor-pointer transition-all duration-300 w-16 h-16">
